...
def compute_var(embeddings, labels, n_way):
    prots = torch.zeros(n_way, embeddings.shape[-1]).type(
        embeddings.dtype).to(embeddings.device)
    intra_var = torch.zeros(n_way).type(
        embeddings.dtype).to(embeddings.device)
    for i in range(n_way):
        if torch.__version__.startswith('1.1'):
            ...
            if (intra!=0).sum()!=0:
                intra_var[i] = intra.sum()/(intra!=0).sum()
            else:
                intra_var[i] = intra.sum()
        else:
            prots[i] = embeddings[(labels == i).nonzero(as_tuple=False), :].mean(0)
            intra_var[i] = embeddings[(labels == i).nonzero(as_tuple=False), :].var(0)
    inter_var = F.cosine_similarity(prots.unsqueeze(1), prots, dim=-1, eps=1e-30).triu(diagonal=-1)
    inter_var = inter_var.sum()/(inter_var!=0).sum()
    ...
    return (intra_var.mean(), inter_var, whole_sim)
